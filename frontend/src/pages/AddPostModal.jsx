...
import { makeStyles, Button, Card, CardHeader,CardContent, Typography, Avatar} from '@material-ui/core';
 ...
 const [postData, setPostData] = useState(null);
  ...
  const handleCloseAddModal = () => {
    ...
    const handleEditPost = async () => {
        try {
            const { data } = await axios.patch(`http://localhost:3000/api/post/${props.postId}`, {
    ...
    <div>
      ...
      >
        ...
        <CardContent>
          ...
          <br/><br/>  
          <textarea  className={classes.textFieldStyle} id="txtPost" type='text' placeholder="Enter Post here...." defaultValue={props.data} rows="2" onChange={handleTextField} autoFocus="true"/>
          </Typography>
        </CardContent>
        </Card>
          : 
        <textarea  className={classes.textFieldStyle} id="txtPost" type='text' placeholder="Enter Post here...." defaultValue={props.data} rows="4" onChange={handleTextField} autoFocus="true"/>
        }
         
        <Button  variant="contained" color='secondary' size='medium' type="reset" defaultValue="Reset" onClick={() => {props.data? handleEditPost() : handleAddPost()}}>Enter</Button>
        <Button  variant="contained" color='primary' size='medium' type="reset" defaultValue="Reset" onClick={handleCloseAddModal}>Cancel</Button>
      </ReactModal>
    </div>
  );
}
